...
const createAuthUserService = async (user) => {
    await db_1.db.insert(schema_1.AuthOnUserTable).values(user);
    return "User created successfully";
};
exports.createAuthUserService = createAuthUserService;
const userLoginService = async (user) => {
    const { username } = user;
    return await db_1.db.query.AuthOnUserTable.findFirst({
        columns: {
            username: true,
            password: true,
            role: true,
        }, where: (0, drizzle_orm_1.sql) `${schema_1.AuthOnUserTable.username} = ${username} `,
        with: {
            user: {
                columns: {
                    name: true,
